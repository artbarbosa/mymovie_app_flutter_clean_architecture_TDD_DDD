...
void main() {
  ...
  final usecase = GetComingSoonUseCaseImp(repository);
  final noParams = NoParams();

  test('getComingSoon must return List MovieEntity', () async {
    when(() => repository.getComingSoon()).thenAnswer((_) async => const Right(<MovieEntity>[]));
    var result = await usecase.call(noParams);
    expect(result.isRight(), true);
    expect(result.fold(id, id), isA<List<MovieEntity>>());
  });
  test('getComingSoon must return ErrorMovie', () async {
    final noParams = NoParams();
    when(() => repository.getComingSoon()).thenAnswer((_) async =>
